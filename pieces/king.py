...
class King(Piece):
    ...
    def CanCastle(piece):
        return piece is not None and piece.previousMove is None
    ...
    def Castle(self, board):
        ...
        if self.previousMove is None:
            # CASTLE LEFT
            if (board.grid[1][self.position.y] is None and board.grid[2][self.position.y] is None
                and board.grid[3][self.position.y] == None) and King.CanCastle(leftRook):
                castles.append(Position(2, self.position.y))
            # CASTLE RIGHT
            if (board.grid[5][self.position.y] is None and board.grid[6][self.position.y] is None) \
